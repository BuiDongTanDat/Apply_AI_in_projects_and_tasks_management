...
class LLMService:
   ...
   # PIPELINE SINH TASK
    ...
    def generate_phases(
        ...
        project_id: int,
        project_spec: str,
        duration: float, # Thời gian dự án, tính theo tháng
        ...
        if not start_date:
            return {"error": "Thiếu start_date."}
        if not duration:
            return {"error": "Thiếu duration."}

        # convert start_date
        try:
            start_date_obj = datetime.strptime(start_date, "%d/%m/%Y")
            start_date_iso = start_date_obj.date().isoformat()
        except ValueError:
            return {"error": "start_date phải có định dạng dd/mm/yyyy"}
    ...
    - Thời gian dự án: {duration} tháng
    Đặc tả dự án:
    {project_spec}
    ...
    - Khoảng 8-10 phase.Mỗi phase là một nhóm công việc lớn của dự án
    - Các phase KHÔNG bắt buộc phải tuần tự, có thể diễn ra song song hoặc overlap thời gian nếu hợp lý
    - Timeline phải nằm trong khoảng thời gian dự án
    ...
    """
        ...
        prompt = PromptTemplate(
            template=template,
            input_variables=["project_spec", "lang", "duration", "start_date"]
        )
    ...
    Yêu cầu: Viết bằng {lang}. Trình bày nhận xét và đề xuất dưới dạng danh sách chỉ mục (ul/li), mỗi phần có tiêu đề in đậm (b). Khi giải thích, hãy kèm dẫn chứng cụ thể từ tasks hoặc comments (ví dụ: trích dẫn tiêu đề task, số lượng task hoàn thành, hoặc nội dung comment).
    Trả về JSON hợp lệ, không bọc bằng ký tự đặc biệt.
    JSON format:
    {{
    "performance_review": "<b>Nhận xét tổng thể</b><ul><li>Hoàn thành 5/7 tasks, ví dụ: 'Fix login bug'</li><li>Chủ động nhận nhiệm vụ khó, như 'Implement payment gateway'</li></ul>",
    "improvement_suggestions": "<b>Đề xuất cải thiện</b><ul><li>Nên cập nhật tiến độ thường xuyên hơn (chưa có comment ở task 'Update documentation')</li><li>Ưu tiên xử lý các task có deadline gần, như 'Refactor API' (deadline: 2026-03-18)</li></ul>"
    }}
    """
