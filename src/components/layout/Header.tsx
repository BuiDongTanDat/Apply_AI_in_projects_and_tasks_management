...
import { notifications as mockNotifications } from '@/db/notifications.mock';
...
export default function AppHeader() {
  ...
  const [notifList, setNotifList] = useState<Notification[]>(mockNotifications || []);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (event.target && (event.target as Element).closest && !(event.target as Element).closest('.search-container')) {
        setShowSearchResults(false);
      }
    };
  ...
  const deleteNotification = (id: string) => setNotifList(prev => prev.filter(n => n.id !== id));

  return (
    <header className={`sticky top-0 z-30 flex items-center justify-between h-14 px-2 gap-3 transition-all duration-300 ${isScrolled
      ? 'bg-primary/9 backdrop-blur-md shadow-lg border border-white/20 px-5'
      : 'bg-transparent'
      }`}>
      {/* Left: toggle + search */}
      <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="relative w-56 md:w-72 lg:w-80 search-container">
          ...
          <Input
            type="text"
            variant="search"
            placeholder="Searching for features..."
            value={searchQuery}
      ...
      <div className="flex items-center gap-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <div className={`flex items-center gap-2 rounded-xl px-2 py-1 cursor-pointer transition-colors border border-gray-100 hover:border-primary ${isScrolled
          ...
          <DropdownMenuContent className="w-52" align="end" sideOffset={8}>
            <DropdownMenuLabel className="flex flex-col">
              <span className="text-sm font-medium">Tan_san</span>
